"""The worked example the interface holds up: a real extraction prompt and a real judging
rubric, from the PET depolymerisation database this toolkit was generalised out of.

They are never used as values. They appear as grey placeholder text in the prompt boxes, so
someone writing their first prompt can see the level of specificity that actually works --
"propagate a footnote condition to every row of that table", not "extract the data carefully"
-- and can copy, adapt or ignore it. A prompt has to be written and saved deliberately before
anything runs, because a prompt silently defaulted to somebody else's chemistry is worse than
no prompt at all.
"""
